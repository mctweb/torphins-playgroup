<template>
  <footer class="bg-white border-blue border-t-4 w-full p-6 md:p-12 lg:p-24 xl:p-32">
    ...
    <div class="flex flex-wrap mx-auto max-w-9xl items-center">
      <section class="flex flex-col font-black order-2 w-full py-6 md:text-lg lg:order-1 lg:text-2xl lg:w-1/2">
        <NuxtLink v-for="(link,i) in links" :key="i" :to="link.to.toLowerCase()" class="mb-1 duration-150 lg:mb-2 hover:text-red">
          {{ link.text }}
        </NuxtLink>
      </section>
...
export default {
  ...
  async fetch () {
    ...
    this.links = normalizeLinks(footer.links, 'to')
